export class App {
  ...
  error(err) {
    ...
    const doc_url =
      "https://github.com/matteo-ronchetti/aws-lambda-power-tuning-ui";
    ...
    switch (err) {
      case "empty":
        ...
        document.getElementById("error-title").innerHTML =
          "Error: URL must contain data in the hash!";
      ...
      case "malformed":
        document.getElementById("error-title").innerHTML =
          "Error: malformed URL parameters!";
        document.getElementById("error-msg").innerHTML =
          "Please check your URL parameters, refer to the <a href='${doc_url}#url-query-string-format'>documentation</a>.";
        break;
      default:
        // general error
  ...
  show(hash, hash2 = null, legend = null) {
    ...
    if (hash2) {
      ...
      // remove duplicates
      powerValues = powerValues.filter(
        (item, pos) => powerValues.indexOf(item) === pos
      );
      // sort numerically
  ...
  showCards(data) {
    ...
    const configurations = data.powers.map((x, i) => {
      return {
        ...
        cost: data.costs[i],
        value:
          (alpha * data.costs[i]) / mc + ((1 - alpha) * data.times[i]) / mt,
      };
    });
    ...
    document.getElementById(
      "min-time"
    ).innerHTML = `${configurations[0].size}MB`;
    document.getElementById("max-time").innerHTML = `${
      configurations[configurations.length - 1].size
    }MB`;
    configurations.sort((x, y) => x.cost - y.cost);
    document.getElementById(
      "min-cost"
    ).innerHTML = `${configurations[0].size}MB`;
    document.getElementById("max-cost").innerHTML = `${
      configurations[configurations.length - 1].size
    }MB`;
...
function customTooltip(tooltipModel) {
  ...
  if (tooltipModel.dataPoints[2] && tooltipModel.dataPoints[2].value) {
    ...
    elTime2.innerHTML = `Time: ${time2}ms (${
      Math.sign(percTime) * percTime
    }% ${timeLabel})`;
    elCost2.innerHTML = `Cost: ${cost2}$ (${
      Math.sign(percCost) * percCost
    }% ${costLabel})`;
